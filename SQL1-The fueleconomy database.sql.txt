...
--Didn't know how to do it until I see the solution. Below are from the solution. 
...
FROM vehicles
WHERE 
	make='Ford' OR make='Chevrolets'
	AND class='Compact Cars' OR class='Two Seaters';
	
--Blow is from the solution. Why did this query get different a result? They read to me the same. 
SELECT COUNT(*) 
FROM vehicles
WHERE
...
--The answer is the results change significantly from a list of make starting from BMW to Scion only. 
SELECT *
FROM vehicles
