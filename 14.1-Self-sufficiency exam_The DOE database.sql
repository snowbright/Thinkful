...
SELECT state, COUNT(avg_math_4_score) AS N, ROUND(AVG(avg_math_4_score), 2) AS mean, 
		ROUND(MIN(avg_math_4_score),2) AS min, ROUND(MAX(avg_math_4_score),2) AS max
FROM naep
GROUP BY state
ORDER BY state; 
...
SELECT state, COUNT(avg_math_4_score) AS N, ROUND(AVG(avg_math_4_score), 2) AS mean,
		ROUND(MIN(avg_math_4_score),2) AS min, ROUND(MAX(avg_math_4_score), 2) AS max, 
	    ROUND((MAX(avg_math_4_score)-MIN(avg_math_4_score)),2) AS diff_max_min
FROM naep
GROUP BY state
...
SELECT ROUND(AVG(avg_math_4_score), 2) AS avg_2000
FROM naep
WHERE year = 2000;
...
SELECT naep.state, ROUND(avg_math_4_score, 2) AS rounded_avg_math_4_score, total_expenditure
